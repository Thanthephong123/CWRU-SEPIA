...
public class Extension_Collection extends Agent {

	private static final long serialVersionUID = -7481143097108592969L;

	public Extension_Collection(int playernum) {
		super(playernum);
    ...
    List<Integer> farmIds = new ArrayList<Integer>();
		...
		for(Integer peasantID : peasantIds)
		{
			...
			// If there are less than 2 farms, build more
			if(farmIds.size() < 2){
				if(currentGold >= 500 && currentWood >= 250)
				{
					...
					actions.put(peasantID, Action.createCompoundBuild(peasantID, farmTemplateID, 9, 14));
				}
			}
